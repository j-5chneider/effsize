...
data_w <- full_join(plot_info, 
    ...
    left_join(readxl::read_excel(here::here("data/data_rating_kris.xlsx")) |> 
                  select(session, contains("_kris"))) |> 
    left_join(readxl::read_excel(here::here("data/data_rating_jue.xlsx")) |> 
                  select(session, contains("_jue"))) |> 
    mutate(uxaxs_1x_rating = ifelse(is.na(uxaxs_1x_rating_kris), 
                                    uxaxs_1x_rating_jue,
                                    uxaxs_1x_rating_kris),
           uxaxs_1y_rating = ifelse(is.na(uxaxs_1y_rating_kris), 
                                    uxaxs_1y_rating_jue,
                                    uxaxs_1y_rating_kris),
           uyaxs_1x_rating = ifelse(is.na(uyaxs_1x_rating_kris), 
                                    uyaxs_1x_rating_jue,
                                    uyaxs_1x_rating_kris),
           uyaxs_1y_rating = ifelse(is.na(uyaxs_1y_rating_kris), 
                                    uyaxs_1y_rating_jue,
                                    uyaxs_1y_rating_kris))
...
# compute z-scores of dependent variables
data_w |> 
    select(value,
           infor,
           diffi,
...
study2_data_ended |> 
    ...
    ggplot(aes(color = factor(answer), fill = factor(answer), x = attention_check)) +
        ...
        theme_minimal() +
    ggtitle("Answers to Understanding Items", "Per Result of Attention Check")
```
...
study2_data |> 
    ...
    ggplot(aes(check_02)) +
        geom_bar() +
        theme_minimal() +
    labs(x="treatment check") +
    ggtitle("Treatment Check", "Benchmark Condition")
```
...
study2_data |> 
    ...
    labs(x="treatment check") +
    facet_wrap(~group)+
    ggtitle("Treatment Check", "Signaling Conditions")
```
...
The heavy left tails indicate clearly non-normality. 
Hence we will use the `student()` family.
...
We have here few divergent transitions which are not problematic as the pp_checks 
...
```{r}
cliffs_delta(effic ~ group, data = study_w_timestamp |> 
           filter(plotNrWithin > 7 & is.infinite(effic) == F,
                  !group == "eg_signaling_overlap",
                  !group == "eg_signaling_u3"
                  )
           ) 
...
sjPlot::tab_model(hyp2b_bernoulli_vinterceptonly,
                  hyp2b_bernoulli_vintercept,
                  show.icc = TRUE)
...
sjPlot::tab_model(hyp7bu3_vinterceptonly,
                  hyp7bu3_vintercept,
                  show.icc = TRUE)
...
## Hypothesis H7bov: Effects of signaling on misconcepts overlap
...
sjPlot::tab_model(hyp7bov_vinterceptonly,
                  hyp7bov_vintercept,
                  show.icc = TRUE)
...
## Statistical understanding and accuracy

### Comprehension of axes
```{r}
#| cache: true

data_undestanding_axes_acc <- 
    data_w |> 
    select(contains("diff_"), uxaxs_1x_rating, uxaxs_1y_rating, 
           uyaxs_1x_rating, uyaxs_1y_rating, 
           session) |> 
    pivot_longer(names_to = "diff_variable",
                 values_to = "diff",
                 cols = c(diff_am, diff_u3, diff_ov)) |> 
    pivot_longer(names_to = "understanding_variable",
                 values_to = "understanding",
                 cols = c(uxaxs_1x_rating, uxaxs_1y_rating, 
                          uyaxs_1x_rating, uyaxs_1y_rating)) |> 
    na.omit() |> 
    distinct()

data_undestanding_axes_acc |> 
    ggplot(aes(as.factor(understanding), diff)) +
    geom_sina(alpha = .1) +
    facet_grid(diff_variable ~ understanding_variable) +
    stat_summary(fun.data = mean_sdl, 
                 fun.args = list(mult = 1), 
                 color = "#8cd000") +
    theme_minimal()
...
### Comprehension of means

```{r}
data_undestanding_mean_acc <- 
    data_w |> 
    select(contains("diff_"), 
           umean_1x_cor, umean_1y_cor, umean_2x_cor, umean_2y_cor,
           session) |> 
    pivot_longer(names_to = "diff_variable",
                 values_to = "diff",
                 cols = c(diff_am, diff_u3, diff_ov)) |> 
    pivot_longer(names_to = "understanding_variable",
                 values_to = "understanding",
                 cols = c(umean_1x_cor, umean_1y_cor, 
                          umean_2x_cor, umean_2y_cor)) |> 
    na.omit() |> 
    distinct()
...
### Comprehension of variance

```{r}
data_undestanding_variance_acc <- 
    data_w |> 
    select(contains("diff_"), uvary_2x_cor, uvary_2y_cor,
           session) |> 
    pivot_longer(names_to = "diff_variable",
                 values_to = "diff",
                 cols = c(diff_am, diff_u3, diff_ov)) |> 
    pivot_longer(names_to = "understanding_variable",
                 values_to = "understanding",
                 cols = c(uvary_2x_cor, uvary_2y_cor)) |> 
    na.omit() |> 
    distinct()

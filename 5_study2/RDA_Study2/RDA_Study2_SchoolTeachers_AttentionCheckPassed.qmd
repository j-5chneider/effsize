...
data_w <- full_join(plot_info, 
    ...
    mutate(rating_am = as.numeric(acccl), # some var need to be defined as
           ...
           #diff_am = ifelse(effsize_am > 0 ,
           #                 rating_am - effsize_am,
           #                 effsize_am - rating_am),
           # actual difference of rating relative to depicted effectsize
           rating_u3_in_d = qnorm(rating_u3/100), 
           rating_ov_in_d = -2*qnorm(0.5*rating_ov/100),
           overunderrating_u3 = ifelse(effsize < 0,
                                       (rating_u3_in_d - effsize)*(-1),
                                       rating_u3_in_d - effsize),
           overunderrating_overlap = ifelse(effsize < 0,
                                       (rating_ov_in_d - effsize)*(-1),
                                       rating_ov_in_d - effsize),
          
          #diff_u3 = ifelse((effsize_u3 - .5) > 0,
          #                 (rating_u3/100 - .5) - (effsize_u3 - .5),
          #                 (effsize_u3 - .5) -  (rating_u3/100 - .5)),
          #diff_am = ifelse(effsize_am > 0 & rating_am > 0 | 
          #                 effsize_am < 0 & rating_am > 0,
          #                 rating_am - effsize_am,
          #                 effsize_am - rating_am),
          #diff_u3_test = case_when(
          #                 ((effsize_u3 - .5) > 0 & (rating_u3 -.5) > 0) |
          #                 ((effsize_u3 - .5) > 0 & (rating_u3 -.5) < 0) |
          #                 ((effsize_u3 - .5) < 0 & (rating_u3 -.5) > 0) 
          #                 ~ (rating_u3/100 - .5) - (effsize_u3 - .5),
          #                 
...
test <-
    data_w |>
    select(rating_u3,effsize_u3, effsize, rating_u3_in_d, overunderrating_u3, rating_ov, effsize_ov, rating_ov_in_d, overunderrating_overlap)

data_w |> 
  ggplot(aes(overunderrating_u3, group)) +
    geom_violin() +
...
data_w |> 
  ggplot(aes(overunderrating_u3, group)) +
    ...
    theme_minimal() +
    labs(title = "Accuracy (U3)",
         subtitle = "Per Benchmarking and U3 Misconcept")


data_w |> 
  ggplot(aes(overunderrating_overlap, group)) +
    geom_violin() +
    geom_jitter(alpha = .05) +
    stat_summary(fun.data = mean_sdl, 
                 fun.args = list(mult = 1)) +
...
skim(
  ...
  select(diffi, infor, value, rating_u3, 
         rating_ov, rating_am, diff_u3, diff_ov,
         diff_am, sensi_binary, plot, session, group) |>

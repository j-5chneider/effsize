...
cramers_v(data_w_filtered_ha$sensi_binary_filtered,
          data_w_filtered_ha$group)
```
...
```{r}
data_w_filtered_hb_u3 <- 
    data_w |> 
           filter(!group == "eg_benchmark",
                  !group == "eg_signaling_overlap"
                  )

data_w_filtered_hb_ov <- 
    data_w |> 
           filter(!group == "eg_benchmark",
                  !group == "eg_signaling_u3"
                  )
...
cramers_v(data_w_filtered_hb_u3$sensi_binary_filtered,
          data_w_filtered_hb_u3$group)

cramers_v(data_w_filtered_hb_ov$sensi_binary_filtered,
          data_w_filtered_hb_ov$group)
...
cramers_v(data_w_filtered_hb_u3$rating_u3_misconcept,
          data_w_filtered_hb_u3$group)
...
```{r}
#| cache: true
data_undestanding_acc <- 
    data_w |> 
    select(contains("diff_"), uxaxs_1x_rating, uxaxs_1y_rating, 
           uyaxs_1x_rating, uyaxs_1y_rating, session) |> 
...
#| results: asis
data_undestanding_acc |> 
  nest_by(diff_variable, understanding_variable) |> 
  mutate(cliff = effsize::cliff.delta(diff ~ understanding, 
                                      data = data)$estimate,
         CI_lower = effsize::cliff.delta(diff ~ understanding, 
                                      data = data)$conf.int["lower"],
         CI_upper = effsize::cliff.delta(diff ~ understanding, 
                                      data = data)$conf.int["upper"],
         CI_sig = sign(CI_lower) == sign(CI_upper)) |>
    select(-data) |> 
    ungroup() |> 
...
cramers_v(data_w_filtered_hb_ov$rating_ov_misconcept,
          data_w_filtered_hb_ov$group)
>>>>>>> e2b7f4ee8040d6f0d0a6915e4fb59fb14e98af74
```

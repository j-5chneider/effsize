...
```{r}
study1_w %>% 
    # recode dependent variables with missconceptions
...
mod_diffi_gardneraltman_halfeye_x <- 
    brm(scale(diffi) ~ dummy_gardneraltman_halfeye_x + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_x,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_gardneraltman_halfeye_x <- 
    brm(scale(infor) ~ dummy_gardneraltman_halfeye_x + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_x,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_gardneraltman_halfeye_x <- 
    brm(scale(value) ~ dummy_gardneraltman_halfeye_x + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_x,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))
...
mod_diffi_gardneraltman_halfeye_y <- 
    brm(scale(diffi) ~ dummy_gardneraltman_halfeye_y + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_gardneraltman_halfeye_y <- 
    brm(scale(infor) ~ dummy_gardneraltman_halfeye_y + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_gardneraltman_halfeye_y <- 
    brm(scale(value) ~ dummy_gardneraltman_halfeye_y + effsize + (1|session), 
                          data = gardneraltman_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

sjPlot::tab_model(mod_accu3_gardneraltman_halfeye_x, mod_accov_gardneraltman_halfeye_x, mod_diffi_gardneraltman_halfeye_x, mod_infor_gardneraltman_halfeye_x, mod_value_gardneraltman_halfeye_x,
mod_accu3_gardneraltman_halfeye_y, mod_accov_gardneraltman_halfeye_y, mod_diffi_gardneraltman_halfeye_y, mod_infor_gardneraltman_halfeye_y, mod_value_gardneraltman_halfeye_y)
...
mod_diffi_gardneraltman_raincloud <- 
    brm(scale(diffi) ~ dummy_gardneraltman_raincloud + effsize + (1|session), 
                          data = gardneraltman_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_gardneraltman_raincloud <- 
    brm(scale(infor) ~ dummy_gardneraltman_raincloud + effsize + (1|session), 
                          data = gardneraltman_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_gardneraltman_raincloud <- 
    brm(scale(value) ~ dummy_gardneraltman_raincloud + effsize + (1|session), 
                          data = gardneraltman_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))
...
mod_diffi_halfeye_x_halfeye_y <- 
    brm(scale(diffi) ~ dummy_halfeye_x_halfeye_y + effsize + (1|session), 
                          data = halfeye_x_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_halfeye_x_halfeye_y <- 
    brm(scale(infor) ~ dummy_halfeye_x_halfeye_y + effsize + (1|session), 
                          data = halfeye_x_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_halfeye_x_halfeye_y <- 
    brm(scale(value) ~ dummy_halfeye_x_halfeye_y + effsize + (1|session), 
                          data = halfeye_x_vs_halfeye_y,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

sjPlot::tab_model(mod_accu3_halfeye_x_halfeye_y, mod_accov_halfeye_x_halfeye_y, mod_diffi_halfeye_x_halfeye_y, mod_infor_halfeye_x_halfeye_y, mod_value_halfeye_x_halfeye_y,mod_accu3_gardneraltman_raincloud, mod_accov_gardneraltman_raincloud, mod_diffi_gardneraltman_raincloud, mod_infor_gardneraltman_raincloud, mod_value_gardneraltman_raincloud)

## Halfeye xaxis vs. Raincloud Plot 
halfeye_x_vs_raincloud <- study1_w %>%
    group_by(session) %>%
    mutate(dummy_halfeye_x_raincloud = 
               case_when(type == "halfeye_xaxis" ~ 0, TRUE ~ 1)) %>%
    filter(type == "halfeye_xaxis" | type == "raincloud_yaxis") %>%
    ungroup()

mod_accu3_halfeye_x_raincloud <- 
    brm(scale(rating_u3) ~ dummy_halfeye_x_raincloud + effsize + (1|session), 
                          data = halfeye_x_vs_raincloud %>% 
                              filter(rating_u3_missconcept == F),
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_accov_halfeye_x_raincloud <- 
    brm(scale(rating_ov) ~ dummy_halfeye_x_raincloud + effsize + (1|session), 
                          data = halfeye_x_vs_raincloud%>% 
                              filter(rating_ov_missconcept == F),
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_diffi_halfeye_x_raincloud <- 
    brm(scale(diffi) ~ dummy_halfeye_x_raincloud + effsize + (1|session), 
                          data = halfeye_x_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_halfeye_x_raincloud <- 
    brm(scale(infor) ~ dummy_halfeye_x_raincloud + effsize + (1|session), 
                          data = halfeye_x_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_halfeye_x_raincloud <- 
    brm(scale(value) ~ dummy_halfeye_x_raincloud + effsize + (1|session), 
                          data = halfeye_x_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

sjPlot::tab_model(mod_accu3_halfeye_x_raincloud, mod_accov_halfeye_x_raincloud, mod_diffi_halfeye_x_raincloud, mod_infor_halfeye_x_raincloud, mod_value_halfeye_x_raincloud)


## Halfeye yaxis vs. Raincloud Plot 
halfeye_y_vs_raincloud <- study1_w %>%
    group_by(session) %>%
    mutate(dummy_halfeye_y_raincloud = 
               case_when(type == "halfeye_yaxis" ~ 0, TRUE ~ 1)) %>%
    filter(type == "halfeye_yaxis" | type == "raincloud_yaxis") %>%
    ungroup()

mod_accu3_halfeye_y_raincloud <- 
    brm(scale(rating_u3) ~ dummy_halfeye_y_raincloud + effsize + (1|session), 
                          data = halfeye_y_vs_raincloud %>% 
                              filter(rating_u3_missconcept == F),
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_accov_halfeye_y_raincloud <- 
    brm(scale(rating_ov) ~ dummy_halfeye_y_raincloud + effsize + (1|session), 
                          data = halfeye_y_vs_raincloud%>% 
                              filter(rating_ov_missconcept == F),
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_diffi_halfeye_y_raincloud <- 
    brm(scale(diffi) ~ dummy_halfeye_y_raincloud + effsize + (1|session), 
                          data = halfeye_y_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_infor_halfeye_y_raincloud <- 
    brm(scale(infor) ~ dummy_halfeye_y_raincloud + effsize + (1|session), 
                          data = halfeye_y_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))

mod_value_halfeye_y_raincloud <- 
    brm(scale(value) ~ dummy_halfeye_y_raincloud + effsize + (1|session), 
                          data = halfeye_y_vs_raincloud,
                          iter = 10000,
                          save_pars = save_pars(all = TRUE))
...
sjPlot::tab_model(mod_accu3_gardneraltman_halfeye_x, mod_accov_gardneraltman_halfeye_x, mod_diffi_gardneraltman_halfeye_x, mod_infor_gardneraltman_halfeye_x, mod_value_gardneraltman_halfeye_x,
mod_accu3_gardneraltman_halfeye_y, mod_accov_gardneraltman_halfeye_y, mod_diffi_gardneraltman_halfeye_y, mod_infor_gardneraltman_halfeye_y, mod_value_gardneraltman_halfeye_y,
mod_accu3_halfeye_x_raincloud, mod_accov_halfeye_x_raincloud, mod_diffi_halfeye_x_raincloud, mod_infor_halfeye_x_raincloud, mod_value_halfeye_x_raincloud,
mod_accu3_halfeye_y_raincloud, mod_accov_halfeye_y_raincloud, mod_diffi_halfeye_y_raincloud, mod_infor_halfeye_y_raincloud, mod_value_halfeye_y_raincloud)
```

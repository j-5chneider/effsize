...
plot01_mod1 <- 
    brm(effic | trunc(lb = 0) ~ 1 + effsize + (1|session),
    data = study1_w_timestamp_effsize %>% 
        filter(plotNrWithin == 1),
    family = "student",
    save_pars = save_pars(all = TRUE)#,
...
plot01_mod2 <- 
    brm(effic | trunc(lb = 0) ~ 1 + effsize + type + (1|session),
    data = study1_w_timestamp_effsize %>% 
        filter(plotNrWithin == 1),
    family = "student",
    save_pars = save_pars(all = TRUE)#,
...
plot0306_mod1 <- 
    brm(effic | trunc(lb = 0) ~ 1 + plotNrWithin_factor + effsize + 
            (1|session),
    data = study1_w_timestamp_effsize %>% 
        filter(plotNrWithin >= 4),
    family = "student",
...
plot_contrasteffects <- plot_ratingu3 + plot_ratingov + plot_diffi + plot_infor + plot_value +
    plot_layout(ncol=3)

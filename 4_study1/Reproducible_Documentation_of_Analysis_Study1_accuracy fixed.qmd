...
study1_w <- full_join(plot_info, item_values, 
    ...
    dplyr::mutate(rating_am = as.numeric(acccl), # some var need to be defined as
                  ...
                  rating_am_in_d = r_to_d(rating_am/100),
...
study1_w %>% 
    ggplot(., aes(effsize_u3, rating_u3, color = rating_u3_misconcept)) +
    geom_jitter() +
    stat_smooth() +
    ylab("Cohen's U3 metric") +
...
study1_w %>% 
    ggplot(., aes(effsize_abs, rating_ov, color = rating_ov_misconcept)) +
    geom_jitter() +
    stat_smooth(method = "lm") +
    ylab("Overlap metric") +
    xlab("Plotted effect size") +
...
study1_w %>% 
    group_by(rating_ov_misconcept, type) %>% 
    do(tau_ov = unlist(cor(.$effsize_abs, .$rating_ov, method = "kendall", 
                           use = "pairwise.complete"))) %>% 
    unnest(tau_ov)
...
study1_w %>% 
    group_by(rating_u3_misconcept, type) %>% 
...
study1_w %>% 
    select(diff_am, diff_u3, diff_ov, rating_u3_misconcept, rating_ov_misconcept,
           session, type) %>% 
    filter(rating_u3_misconcept == F,
           rating_ov_misconcept == F) %>%
    gather(dependent_variable, difference_to_true_effsize, 
           diff_am, diff_u3, diff_ov) %>% 
    ...
    facet_wrap(~dependent_variable, scales = "free_y", 
                labeller = as_labeller(c("diff_am" = "Abstract metric",
                                         "diff_ov" = "Overlap metric (filtered)",
                                         "diff_u3" = "Cohen's U3 metric (filtered)"))) +
...
mod1_diff_u3 <- brm(diff_u3 ~ effsize + (1|session), 
                          data = study1_w%>% 
                              filter(rating_u3_misconcept == F,
                                     !diff_u3 == "-Inf",
                                     !diff_u3 == "Inf"
...
mod2_diff_u3 <- brm(diff_u3 ~ type + effsize + (1|session), 
                       data = study1_w %>% 
                           filter(rating_u3_misconcept == F,
                                     !diff_u3 == "-Inf",
                                     !diff_u3 == "Inf"),
...
mod1_diff_ov <- brm(diff_ov ~ effsize + (1|session), 
                          data = study1_w%>% 
                              filter(rating_ov_misconcept == F,
                                     !diff_ov == "-Inf",
                                     !diff_ov == "Inf"),
...
mod2_diff_ov <- brm(diff_ov ~ type + effsize + (1|session), 
                          data = study1_w %>% 
                              filter(rating_ov_misconcept == F,
                                     !diff_ov == "-Inf",
                                     !diff_ov == "Inf"),
...
ggplot(study1_w %>% 
    ...
    theme_modern_rc() +
    theme(strip.text = element_text(color = "white")) +
    ggtitle("Sensitivity", "without misconceptualizers")
...
ggsave("4_study1/plot_diffov.png", 
    plot_diffov,
    width = 25*.55,
    height = 16*.55)
...
# create dummy variables to compare visualization types for those measures that showed evidence for difference (accuracy overlap, perceived task difficulty, perceived informativity, perceived value, sensitivity, efficiency)
...
mod_diffov_gardneraltman_halfeye_x <- 
    brm(diff_ov ~ dummy_gardneraltman_halfeye_x +  effsize + (1|session), 
...
mod_diffov_gardneraltman_halfeye_y <- 
    brm(diff_ov ~ dummy_gardneraltman_halfeye_y + effsize + (1|session), 
...
mod_diffov_raincloud_halfeye_x <- 
    brm(diff_ov ~ dummy_raincloud_halfeye_x + effsize + (1|session), 
...
mod_diffov_raincloud_halfeye_y <- 
    brm(diff_ov ~ dummy_raincloud_halfeye_y + effsize + (1|session), 
...
mod_diffov_gardneraltman_raincloud <- 
    brm(diff_ov ~ dummy_gardneraltman_raincloud + effsize + (1|session), 
...
mod_diffov_halfeye_x_halfeye_y <- 
    brm(diff_ov ~ dummy_halfeye_x_halfeye_y + effsize + (1|session), 
...
```{r}
sjPlot::tab_model(mod_diffov_gardneraltman_raincloud, 
                  mod_diffi_gardneraltman_raincloud,
                  mod_infor_gardneraltman_raincloud, 
                  mod_value_gardneraltman_raincloud,
                  mod_sensi_gardneraltman_raincloud,
...
```{r}
sjPlot::tab_model(mod_diffov_halfeye_x_halfeye_y, 
                  mod_diffi_halfeye_x_halfeye_y, 
                  mod_infor_halfeye_x_halfeye_y, 
                  mod_value_halfeye_x_halfeye_y,
                  mod_sensi_halfeye_x_halfeye_y,

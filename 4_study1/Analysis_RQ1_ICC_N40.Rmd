...
study1_plot <-  study1_w %>%
    dplyr::select(session, plot, type, acccl, accu3, accov, diffi, infor, value) %>%
    pivot_longer(acccl:value, names_to = "variables", values_to = "values", values_transform = as.character) %>%
    mutate(var = paste(variables, type, sep = "_"))%>%
    pivot_wider(id_cols = c(session, plot),names_from = "var", values_from = "values") %>%
    dplyr::select(-plot) 

skim(study1_plot)

study1_plot_accuracy <- study1_plot %>%
    dplyr::select(session, acccl_gardneraltman_xaxis, acccl_halfeye_xaxis, acccl_raincloud_yaxis, acccl_halfeye_yaxis)


### Multiple Imputation of Planned Missings
library(mice)
set.seed(2001)
M_plot <- 5
out_plot <- mice(data = study1_plot_accuracy%>%
                     dplyr::select(-starts_with("session")),
            m = M_plot,
